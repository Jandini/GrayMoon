...
- **Stage, commit, and undo across every repo from one screen.** Git Changes shows staged and unstaged files for the whole workspace in a single multi-repo tree with a diff viewer (including rendered markdown Preview with Changes / After / Before for `.md` files, plus Mermaid pan/zoom), multi-select stage/unstage, a commit split-button (with optional push), and discard/undo - no more `git status` in a dozen terminals.
