...
| **ReturnToDefaultBranch** | Switch to default branch, optionally delete previous branch if merged, pull. | Branch endpoints (return to default). | `workspaceName`, `workspaceRoot`, `repositoryName`, `currentBranchName`, `bearerToken` | Get default branch, checkout, delete old local branch if not default (force=false), pull. |
