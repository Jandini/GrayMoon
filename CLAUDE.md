...
The App sends commands to the Agent over SignalR using a `requestId` (GUID). `AgentBridge` calls `AgentResponseDelivery.WaitAsync(requestId)` (a `TaskCompletionSource`) then fires `RequestCommand` to the Agent. The Agent enqueues a `JobEnvelope` into one of three bounded `Channel<JobEnvelope>` pools (main/read/diff, selected by command name), runs it on that pool's dedicated workers, and calls back `ResponseCommand` with the same `requestId`. `AgentResponseDelivery.Complete` resolves the TCS, unblocking the App's awaiter. Intermediate output streams via `CommandOutput` events for the loading overlay terminal.

### Agent → App sync (hook-driven)

Git hooks (`post-commit`, `post-checkout`, `post-merge`, `pre-push`) POST JSON to the Agent's local HTTP listener (`/hook/commit`, `/hook/checkout`, `/hook/merge`, or `/hook/push`). The Agent processes these as `NotifySyncJob` (`INotifyJob`) jobs and pushes partial state updates back to the App via `SyncCommand` on the SignalR connection. `SyncCommandHandler` persists only non-null fields (partial update semantics) then broadcasts `WorkspaceSynced` to all browser clients.
...
- **Change detection is watcher-driven, not polled from the browser.** Git Changes repository watchers are kept warm for the currently active GrayMoon workspace, independent of which page within that workspace is open. Workspaces that are no longer active age out through the configured workspace grace period and Agent watcher idle grace period. On the Agent, `GitRepositoryWatcherManager` lease-manages one `FileSystemWatcher` per repo (`GitRepositoryWatcher`) once the App has asked about that repo via the `GetGitChangeStatus` command. Watcher events are invalidation-only signals into `GitStatusRefreshCoordinator`, which debounces/coalesces them (state machine: `Clean`/`Dirty`/`Refreshing`/`RefreshingAndDirty`) behind a shared semaphore (`GitChangesOptions.MaxParallelRepositoryOperations`, default 8) so at most one scan plus one coalesced follow-up runs per repo at a time. Working-tree watcher events also record their UTC observation time (path/kind) as Agent-local coverage metadata while remaining invalidation-only for Git state. Each scan shells out to `git status --porcelain=v2 -z --branch --untracked-files=all`, parsed by the process-free `GitPorcelainV2Parser` (Common). `GitChangesSnapshotCache` versions each result; `GitChangesSnapshotPublisher` pushes new snapshots to the App over the existing Agent-App SignalR connection.
