...
- **GrayMoon.App** (`src/GrayMoon.App`) — ASP.NET Core **.NET 10** + Blazor Server, runs in Docker. Exposes the web UI and two always-on SignalR hubs (`/hub/agent` for App↔Worker commands, `/hubs/workspace-sync` for browser broadcast); in desktop mode a third hub, `/hubs/desktop` (`DesktopNotificationHub`), is also mapped. Never touches the local filesystem or runs git directly. SQLite database via EF Core at `/app/db` (volume-mounted). Uses `EnsureCreated()` for brand-new databases, plus guarded idempotent migrations in `Migrations.cs` for existing ones - see "Database schema" below; migrations are always required, not just post-release. **New DB columns must be nullable or have a default value.**
- **GrayMoon.Agent** (`src/GrayMoon.Agent`) — .NET **10** console app / dotnet tool (`graymoon-worker` command) that runs on the developer's host machine. Packaged as a single binary; supports Windows Service (`Microsoft.Extensions.Hosting.WindowsServices`) and Linux Systemd (`Microsoft.Extensions.Hosting.Systemd`) hosting. CLI entry via `AgentCli.Build()` with subcommands: `run` (default), `install`, `uninstall`, `start`, `stop`. Connects to the App via SignalR, executes all git and filesystem operations, and exposes a local HTTP listener on `127.0.0.1:9191` for git hook callbacks. Job queue: three separate bounded `Channel<JobEnvelope>` pools, routed by command name so status scans and diffs never queue behind long-running writes - the main pool (`AgentOptions.MaxConcurrentCommands`, default `Environment.ProcessorCount * 2`) for most commands, a dedicated read pool (`MaxConcurrentReadCommands`, default 8) for `GetGitChangeStatus`, and a dedicated diff pool (`MaxConcurrentDiffCommands`, default 4) for `GetGitFileDiff`. Hosted services manage the Agent lifecycle: `SignalRConnectionHostedService` (maintains hub connection, routes each `RequestCommand` to the right pool), `HookListenerHostedService` (HTTP listener for git hook POSTs), and `MainJobBackgroundService`/`ReadJobBackgroundService`/`DiffJobBackgroundService` (each extends the abstract `JobBackgroundService`, dequeuing and executing jobs from its own pool).
- **GrayMoon.Abstractions** (`src/GrayMoon.Abstractions`) — Shared interfaces and DTOs used by both App and Agent. `Agent/AgentHubMethods.cs` contains all SignalR hub method name constants (`RequestCommand`, `CancelCommand`, `ResponseCommand`, `CommandOutput`, `SyncCommand`, `ReportSemVer`, `ReportQueueStatus`, `SelfUpdate`, `CheckFileVersions`, `ResolveGeneratedPackageReferences`, `GitChangesSnapshotUpdated`).
- **GrayMoon.Application** (`src/GrayMoon.Application`) — Application-layer contracts used by Blazor pages, thin REST endpoints, and later automation: `IWorkspace*Operations` facades, `OperationProgress` / `OperationResult`, and shared operation DTOs. Does not reference `GrayMoon.App`. Implementations live in `src/GrayMoon.App/Services/Application/`.
...
### App → Worker command flow

The App sends commands to the Worker over SignalR using a `requestId` (GUID). `AgentBridge` calls `AgentResponseDelivery.WaitAsync(requestId)` (a `TaskCompletionSource`) then fires `RequestCommand` to the Worker. The Worker enqueues a `JobEnvelope` into one of three bounded `Channel<JobEnvelope>` pools (main/read/diff, selected by command name), runs it on that pool's dedicated workers, and calls back `ResponseCommand` with the same `requestId`. `AgentResponseDelivery.Complete` resolves the TCS, unblocking the App's awaiter. Intermediate output streams via `CommandOutput` events for the loading overlay terminal.

### Worker → App sync (hook-driven)

Git hooks (`post-commit`, `post-checkout`, `post-merge`, `pre-push`) POST JSON to the Worker's local HTTP listener (`/hook/commit`, `/hook/checkout`, `/hook/merge`, or `/hook/push`). The Worker processes these as `NotifySyncJob` (`INotifyJob`) jobs and pushes partial state updates back to the App via `SyncCommand` on the SignalR connection. `SyncCommandHandler` persists only non-null fields (partial update semantics) then broadcasts `WorkspaceSynced` to all browser clients.

### Worker command structure

The Worker uses `System.CommandLine`. Each worker operation implements `ICommandHandler<TRequest, TResponse>` in `src/GrayMoon.Agent/Commands/`. See the `add-agent-command` skill for the step-by-step recipe to add a new one.
...
`GhaWorkflowLiveFeedService` polls `GetWorkflowRunJobsAsync` (jobs + steps) on adaptive intervals (4 s active, 8 s waiting for jobs, 15 s idle). It tracks step-status transitions and feeds the step board of `GhaWorkflowLiveTerminal`. Rate-limit backoff state lives in `IGitHubRateLimitTracker` (singleton, keyed by connector name), not a field on the service itself - this matters because `WorkspacePushService` resolves its own `GhaWorkflowLiveFeedService` from a separate DI scope (via `ScopedExecutor`/`ServiceScopeFactory.CreateAsyncScope()`) during a push, so a per-instance field would not have been shared with the terminals on the page; routing the gate through the tracker singleton means a 429 hit by any poller for a connector pauses every poller for that connector, regardless of scope.
...
- Worker commands `CheckFileVersions` and `UpdateFileVersions` live in `src/GrayMoon.Agent/Commands/`.
...
- **Change detection is watcher-driven, not polled from the browser.** On the Worker, `GitRepositoryWatcherManager` lease-manages one `FileSystemWatcher` per repo (`GitRepositoryWatcher`) once the App has asked about that repo via the `GetGitChangeStatus` command. Watcher events are invalidation-only signals into `GitStatusRefreshCoordinator`, which debounces/coalesces them (state machine: `Clean`/`Dirty`/`Refreshing`/`RefreshingAndDirty`) behind a shared semaphore (`GitChangesOptions.MaxParallelRepositoryOperations`, default 8) so at most one scan plus one coalesced follow-up runs per repo at a time. Each scan shells out to `git status --porcelain=v2 -z --branch --untracked-files=all`, parsed by the process-free `GitPorcelainV2Parser` (Common). `GitChangesSnapshotCache` versions each result; `GitChangesSnapshotPublisher` pushes new snapshots to the App over the existing Worker-App SignalR connection.
...
Schema is owned by EF Core but applied via `EnsureCreated()`, which only creates a brand-new database file from the current model - it never alters an existing one. Developers, testers, and users routinely already have a local `graymoon.db` from an earlier build (this is true even pre-release), so any schema change that isn't paired with a migration will throw `no such table`/`no such column` for everyone with an existing database, exactly like a real production incident.

**Migrations are always required, pre-release or not.** Every schema change needs both:
1. The entity class update and the corresponding `modelBuilder.Entity<T>(...)` block in `AppDbContext.OnModelCreating` (`src/GrayMoon.App/Data/AppDbContext.cs`) - this is what `EnsureCreated()` uses for brand-new databases.
2. A new idempotent `public static async Task MigrateXxxAsync(AppDbContext dbContext)` method added to `src/GrayMoon.App/Migrations.cs` (guarded by `pragma_table_info('TableName') WHERE name='ColumnName'` or `sqlite_master` checks before running `ALTER TABLE`/`CREATE INDEX`/etc.), called at the end of `RunAllAsync`, so already-installed databases get patched at next startup.
