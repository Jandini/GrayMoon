...
public interface ICsProjFileParser
{
    ...
    /// <summary>
    /// Finds the PackageReference whose element text contains a match for the given version-pattern
    ...
    /// Returns the Include name and the Version captured between prefix and suffix, or null if the file is
    /// missing/invalid or no PackageReference element matches.
    /// </summary>
    Task<PackageReferenceMatch?> FindPackageReferenceForVersionPatternAsync(string csprojPath, string prefix, string suffix, CancellationToken cancellationToken = default);
}
